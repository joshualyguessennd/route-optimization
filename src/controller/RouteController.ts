...
export class RouteController {
    ...
    async findRoutes(params: {
        ...
        tokenAddress: string;
        userAddress: string;
    }, headers: { 'x-scenario-type'?: string }) {
        try {
            // Validate parameters
            ...
            // Get scenario type from headers
            const scenario = headers['x-scenario-type'] === 'scenario2' ? 'scenario2' : 'scenario1';

            // Create RouteOptimizer with scenario
            ...
            // Get optimal routes
            const routes = await routeOptimizer.findOptimalRoutes(
                params.targetChain,
                params.amount,
                params.userAddress
            );
    ...
    setup(app: Elysia): Elysia {
        return app
            .post(
                '/api/route',
                async ({ body, headers }) => this.findRoutes(body, headers as Record<string, string>),
                {
                    body: t.Object({
